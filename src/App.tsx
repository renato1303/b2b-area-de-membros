...
export default function App() {
  // Madeleine - Administradora Oficial Será Cacau
  const madeleineProfile: UserProfile = {
    ...
    couponCode: 'B2BATACADO',
    totalPoints: 0,
    tier: 'Diamante'
  };

  // State
  const [currentUser, setCurrentUser] = useState<UserProfile>(madeleineProfile);
  ...
  useEffect(() => {
    ...
    // Limpar quaisquer cadastros/membros de testes anteriores para garantir 0 cadastros
    try {
      localStorage.removeItem('sera_cacau_registered_nutris');
      localStorage.removeItem('sera_cacau_members');
    } catch (e) {
      // ignore
    }
    ...
    if (!isSupabaseConfigured) {
      ...
      if (auth && savedUser) {
        try {
          ...
          // Permitir apenas Madeleine como sessão salva localmente
          if (parsed.email === 'madeleine@seracacau.com.br' || parsed.role === UserRole.ADMIN) {
          ...
          .then(({ data: profile }) => {
            if (profile) {
              const mappedProfile: UserProfile = {
                id: profile.id,
                name: profile.name,
                tradeName: profile.trade_name || profile.name,
                companyName: profile.company_name || '',
                cnpj: profile.cnpj || '',
                storeType: profile.store_type || 'Empório & Produtos Naturais',
                wholesaleTier: profile.wholesale_tier || 'Ouro',
                email: profile.email,
                phone: profile.phone || '',
                instagram: profile.instagram || '',
                specialty: profile.specialty || (profile.role === 'ADMIN' ? 'Gestão Comercial & Expansão B2B' : 'Revendedor Autorizado'),
                city: profile.city || '',
                state: profile.state || '',
              ...
              const fallbackProfile: UserProfile = {
                id: session.user.id,
                name: session.user.user_metadata?.name || session.user.email?.split('@')[0] || 'Revendedor',
                tradeName: session.user.user_metadata?.trade_name || session.user.user_metadata?.name || 'Revendedor',
                companyName: session.user.user_metadata?.company_name || '',
                cnpj: session.user.user_metadata?.cnpj || '',
                storeType: session.user.user_metadata?.store_type || 'Empório & Produtos Naturais',
                wholesaleTier: session.user.user_metadata?.wholesale_tier || 'Ouro',
                email: session.user.email || '',
                phone: session.user.user_metadata?.phone || '',
                instagram: session.user.user_metadata?.instagram || '',
                specialty: session.user.user_metadata?.specialty || 'Revendedor Autorizado',
                city: session.user.user_metadata?.city || '',
                state: session.user.user_metadata?.state || '',
                role: session.user.user_metadata?.role || UserRole.NUTRICIONISTA,
                patientCoupon: session.user.user_metadata?.patient_coupon || 'B2BATACADO',
                couponCode: session.user.user_metadata?.coupon_code || 'B2BATACADO',
                totalPoints: 0,
                tier: 'Ouro'
              };
  ...
  // Load registered members/retailers from Supabase profiles
  useEffect(() => {
    if (isSupabaseConfigured) {
      supabase
        .from('profiles')
  ...
  const [notifications, setNotifications] = useState([
    ...
    { 
      id: 'notif-2', 
      title: 'Tabela de Atacado Atualizada', 
      desc: 'Lotes de Cacau em Gotas 100% com 30% de desconto para revenda disponíveis.', 
      time: 'Há 2h', 
      read: false, 
      category: 'Atacado',
      tabId: 'produtos'
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-surface text-primary-text rounded-xl shadow-2xl border border-border-color overflow-hidden z-50 p-2 animate-in fade-in slide-in-from-top-2 duration-200">
                {!searchQuery ? (
                  ...
                  <div className="p-4 text-center text-xs text-secondary-text/50 flex flex-col items-center gap-2">
                    ...
                    <span>Nenhum produto encontrado para "{searchQuery}"</span>
                  </div>
                ) : (
