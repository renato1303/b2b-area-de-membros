    ...
    <div className="px-6 md:px-12 py-8 max-w-5xl mx-auto w-full font-sans text-primary-forest">
      ...
      <div className="flex flex-col gap-2 mb-10 border-b border-border-color/60 pb-5">
        <span className="text-[10px] tracking-[0.3em] uppercase text-primary-accent font-bold font-mono">
          Programa de Bonificação & Parceria B2B
        </span>
        <h2 className="text-3xl font-extrabold tracking-tight text-primary-forest flex items-center gap-2">
          ...
          <span>Pontos Comerciais & Recompensas</span>
        </h2>
        <p className="text-xs text-secondary-text max-w-xl">
          Acumule pontos em cada pedido de atacado faturado, conclusão de módulos de treinamento da sua equipe de balcão e ações de degustação em loja. Desbloqueie bonificações em estoque, displays de madeira nobre e visitas guiadas à fazenda agroflorestal.
        </p>
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div className="flex-1">
            <span className="text-[9px] tracking-widest uppercase text-secondary-text/60 font-mono font-bold block mb-2">
              Status Comercial do Lojista
            </span>
            <div className="flex flex-wrap items-baseline gap-4 mb-4">
              <h3 className="text-2xl font-black text-primary-forest">{user.tradeName || displayName}</h3>
              <span className={`text-[10px] font-mono uppercase font-bold px-3 py-1 rounded-full border ${currentTierStyle.bg} ${currentTierStyle.text} ${currentTierStyle.border}`}>
                Parceiro Nível {tier}
              </span>
            </div>
            ...
            <div className="mt-6 max-w-lg">
              <div className="flex justify-between text-[10px] font-mono font-bold text-secondary-text uppercase tracking-wider mb-2">
                <span>{points} PONTOS B2B ACUMULADOS</span>
                {nextTier ? (
                  <span>FALTAM {nextLimit - points} PTS PARA NÍVEL {nextTier}</span>
                ) : (
                  <span>VOCÊ ALCANÇOU O NÍVEL MÁXIMO DIAMANTE!</span>
                )}
              </div>
              ...
              <div className="flex justify-between text-[9px] font-mono text-secondary-text/50 mt-1.5">
                ...
                {nextTier && <span>{nextLimit} PTS ({nextTier})</span>}
              </div>
            </div>
          </div>
      ...
      {/* REWARDS CATALOG / RESGATE B2B */}
      {rewards && rewards.length > 0 && (
        <div className="flex flex-col gap-5 mb-10">
          <div className="flex items-center justify-between border-b border-border-color/60 pb-3">
            <h3 className="text-lg font-bold tracking-tight text-primary-forest flex items-center gap-2">
              <Gift className="w-5 h-5 text-luxury-accent" />
              <span>Catálogo de Bonificações para o PDV</span>
            </h3>
            <span className="text-[10px] text-secondary-text/60 font-mono uppercase tracking-wider">
              Resgate Direto via Suporte B2B
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {rewards.map((reward) => {
              const IconComp = rewardIcons[reward.icon] || Gift;
              const canRedeem = points >= reward.pointsCost;

              return (
                <div 
                  key={reward.id}
                  className="bg-surface border border-border-color rounded-xl p-5 flex flex-col justify-between shadow-xs hover:border-primary-accent/30 transition-all group"
                >
                  <div className="space-y-3">
                    <div className="w-10 h-10 rounded-xl bg-luxury-accent/15 text-luxury-accent flex items-center justify-center group-hover:scale-105 transition-transform">
                      <IconComp className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-primary-forest leading-snug">
                        {reward.title}
                      </h4>
                  ...
                  <div className="pt-4 border-t border-border-color/60 flex items-center justify-between mt-4">
                    ...
                    <span className={`text-[9px] font-mono font-bold uppercase px-2 py-0.5 rounded ${
                      canRedeem 
                        ...
                        : 'bg-black/5 text-secondary-text/60'
                    }`}>
