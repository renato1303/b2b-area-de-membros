...
export const LibraryView: React.FC<LibraryViewProps> = ({
  ...
  const categories = ['Todos', 'Catálogos', 'PDV', 'Laudos', 'Receitas'];
  ...
  const handleDownload = (fileName: string) => {
    onDownloadIncrement();
    alert(`Iniciando download do arquivo comercial:\n${fileName}\n\nDocumento oficial para revendedores homologado pela Será Cacau B2B.`);
  };
    ...
    <div className="max-w-4xl mx-auto py-8 px-6 md:px-12 flex flex-col gap-10 font-sans text-primary-text">
      ...
      <header className="flex flex-col gap-3">
        <span className="text-xs tracking-[0.2em] uppercase text-primary-accent font-bold font-mono">
          Material Corporativo & Atacado
        </span>
        <h1 className="text-3xl font-extrabold tracking-tight text-primary-forest">
          Central de Arquivos & Suporte PDV
        </h1>
        <p className="text-xs md:text-sm text-secondary-text max-w-xl leading-relaxed">
          Baixe catálogos com tabela de revenda com 30% de desconto e precificação sugerida, fichas técnicas de pureza para clientes exigentes e artes prontas para displays de balcão.
        </p>
      ...
      <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4">
        ...
        <div className="relative w-full md:w-72">
          <input
            type="text"
            placeholder="Buscar catálogos, fichas e PDV..."
            value={search}
