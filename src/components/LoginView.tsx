...
export const LoginView: React.FC<LoginViewProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        try {
          ...
          if (profile && !error) {
            ...
            const mappedProfile: UserProfile = {
              ...
              email: profile.email,
              phone: profile.phone || '',
              instagram: profile.instagram || (cleanEmail.includes('madeleine') ? '@seracacau' : ''),
              specialty: profile.specialty || (isExplicitAdmin ? "Gestor Comercial B2B" : 'Revendedor Autorizado'),
              city: profile.city || '',
              state: profile.state || '',
              ...
              storeType: profile.store_type || 'Empório & Produtos Naturais',
              wholesaleTier: profile.wholesale_tier || 'Nível Ouro',
              patientCoupon: profile.patient_coupon || profile.coupon_code || 'B2BATACADO',
              couponCode: profile.coupon_code || profile.patient_coupon || 'B2BATACADO',
              totalPoints: profile.total_points ?? 0,
              tier: profile.tier || 'Ouro'
            };
            setSuccessMsg(`Bem-vindo, ${mappedProfile.tradeName || mappedProfile.name}! Redirecionando...`);
            setTimeout(() => {
      ...
      // 2. Administradora Autorizada: Madeleine (S'era Cacau)
      if (cleanEmail === 'madeleine@seracacau.com.br' || cleanEmail.includes('madeleine')) {
        const madeleineProfile: UserProfile = {
          name: 'Madeleine',
          tradeName: "S'era Cacau Brasil",
          companyName: "S'era Cacau Agroflorestal Ltda",
          email: 'madeleine@seracacau.com.br',
          phone: '(11) 98123-4567',
          instagram: '@seracacau',
          specialty: "Gestão Comercial & Expansão B2B",
          city: 'São Paulo',
          state: 'SP',
          role: UserRole.ADMIN,
          cnpj: '45.123.890/0001-12',
          storeType: 'Matriz / Indústria Agroflorestal',
          wholesaleTier: 'Embaixador',
          totalPoints: 0,
          tier: 'Diamante'
        };
      ...
      // 3. Check in dynamic registered members (localStorage) and static MEMBERS list
      ...
      if (matchedMember) {
        const memberProfile: UserProfile = {
          ...
          tradeName: matchedMember.tradeName || matchedMember.name,
          email: matchedMember.email,
          phone: matchedMember.phone || '',
          instagram: '',
          specialty: matchedMember.storeType ? `${matchedMember.wholesaleTier || 'Ouro'} • ${matchedMember.storeType}` : (matchedMember.specialty || 'Revendedor Autorizado'),
          city: matchedMember.city || '',
          state: matchedMember.state || '',
          role: UserRole.NUTRICIONISTA,
          crn: matchedMember.crn || '',
          cnpj: matchedMember.cnpj || '',
          storeType: matchedMember.storeType || 'Empório & Cafeteria',
          wholesaleTier: matchedMember.wholesaleTier || 'Ouro',
          patientCoupon: matchedMember.patientCoupon || matchedMember.couponCode || 'B2BATACADO',
          couponCode: matchedMember.couponCode || matchedMember.patientCoupon || 'B2BATACADO',
          totalPoints: matchedMember.totalPoints || 0,
          tier: matchedMember.tier || 'Ouro'
        };
        setSuccessMsg(`Bem-vindo, ${memberProfile.tradeName || memberProfile.name}! Redirecionando...`);
        setTimeout(() => {
      ...
      // 4. IF NOT REGISTERED IN SUPABASE OR DATABASE:
      ...
      <div className="w-full max-w-md bg-surface border border-border-color rounded-2xl p-6 md:p-8 flex flex-col gap-6 shadow-sm relative z-10 my-8">
        ...
        {unregisteredRedirect && (
          <div className="bg-amber-500/10 border border-amber-500/30 text-amber-900 p-4 rounded-xl text-xs flex flex-col gap-3 animate-fadeIn">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <p className="font-bold text-amber-950">E-mail de revendedor não localizado</p>
                <p className="text-amber-800/90 mt-0.5 text-[11px] leading-relaxed">
                  O e-mail <strong className="font-semibold text-amber-950">{unregisteredRedirect.email}</strong> ainda não consta na nossa base de lojistas homologados.
                </p>
              </div>
            </div>
            
            <p className="text-[11px] text-amber-800 leading-normal">
              Redirecionando você para o credenciamento de novos parceiros comerciais...
            </p>
            ...
            >
              <span>Solicitar Credenciamento B2B</span>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                autoFocus
                placeholder="contato@emporioouloja.com.br"
          ...
          <div className="flex items-center gap-2 px-1 text-[10px] text-secondary-text/80 font-mono">
            ...
            <span>Acesso direto via e-mail corporativo homologado.</span>
          </div>
        ...
        <div className="flex flex-col items-center gap-1.5 pt-1 text-center">
          <p className="text-[11px] text-secondary-text">
            Deseja revender Será Cacau em seu estabelecimento?
          </p>
          <a
            href={REGISTRATION_URL}
            target="_blank"
          ...
          >
            <span>Cadastre sua loja como Parceiro Revendedor</span>
        ...
        <div className="text-center pt-2 border-t border-border-color/40">
          <span className="text-[8px] tracking-[0.2em] text-secondary-text/40 uppercase font-mono">
            Agrofloresta Cabruca · Pureza 100% · Alta Rentabilidade PDV
          </span>
        </div>
