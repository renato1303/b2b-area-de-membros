...
export const AdminHeader: React.FC<AdminHeaderProps> = ({
  ...
  const tabs = [
    { id: 'products' as AdminTab, label: 'Catálogo de Produtos', count: stats.totalProducts, icon: ShoppingBag, color: 'text-amber-700' },
    { id: 'members' as AdminTab, label: 'Cadastro de Lojistas', count: stats.totalMembers, icon: Users, color: 'text-indigo-700' },
    { id: 'sheets' as AdminTab, label: 'Fichas & Laudos Técnicos', count: stats.totalSheets, icon: ShieldCheck, color: 'text-teal-700' },
    { id: 'recipes' as AdminTab, label: 'Receitas & Cafeteria', count: stats.totalRecipes, icon: Utensils, color: 'text-luxury-accent' },
    { id: 'science' as AdminTab, label: 'Ciência & Terroir', count: stats.totalArticles, icon: Microscope, color: 'text-sky-700' },
    { id: 'blog' as AdminTab, label: 'Blog & Tendências Varejo', count: stats.totalPosts, icon: FileText, color: 'text-emerald-700' }
  ];
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 border border-white/15 text-xs text-[#E1D7C6] font-medium tracking-wide">
              ...
              <span>Painel de Gestão B2B · Será Cacau</span>
            ...
            <h1 className="text-2xl sm:text-4xl font-serif text-white font-normal tracking-tight">
              Gestão Comercial & Conteúdo B2B
            </h1>

            <p className="text-[#C2C9C0] text-sm sm:text-base leading-relaxed font-light">
              Publique e gerencie o catálogo atacado, fichas técnicas, laudos, tabelas nutricionais e a homologação de lojistas e revendedores em tempo real.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 shrink-0 bg-white/5 p-3.5 rounded-2xl border border-white/10 backdrop-blur-md">
            <div className="p-2.5 rounded-xl bg-white/5 text-center">
              <span className="block text-xl font-serif font-bold text-white">{stats.totalProducts}</span>
              <span className="text-[10px] text-[#A6B2A8] uppercase tracking-wider font-mono">Produtos</span>
            </div>
            <div className="p-2.5 rounded-xl bg-white/5 text-center">
              <span className="block text-xl font-serif font-bold text-white">{stats.totalMembers}</span>
              <span className="text-[10px] text-[#A6B2A8] uppercase tracking-wider font-mono">Lojistas</span>
            </div>
            <div className="p-2.5 rounded-xl bg-white/5 text-center">
            ...
            <div className="p-2.5 rounded-xl bg-white/5 text-center">
              <span className="block text-xl font-serif font-bold text-white">{stats.totalSheets}</span>
              <span className="text-[10px] text-[#A6B2A8] uppercase tracking-wider font-mono">Fichas/Laudos</span>
            </div>
          </div>
        </div>
      </div>
