...
export const AdminImportMembersModal: React.FC<AdminImportMembersModalProps> = ({
  ...
  const handleSyncToSupabase = async () => {
    ...
    try {
      ...
      if (isSupabaseConfigured) {
        // Prepare payload for Supabase 'profiles' table
        const payload = validRows.map(r => ({
          name: r.fullName,
          trade_name: r.fullName,
          email: r.email,
          phone: r.phone || '',
          city: r.city || '',
          state: r.state || '',
          specialty: r.specialty || 'Revendedor Autorizado',
          role: 'REVENDEDOR',
          store_type: 'Empório & Produtos Naturais',
          wholesale_tier: 'Ouro',
      ...
      if (missingTableDetected) {
        ...
        setSyncFeedback({
          type: 'error',
          message: `⚠️ Os ${validRows.length} revendedores foram carregados no portal com sucesso! Porém a tabela "profiles" ainda não existe no seu projeto Supabase (Erro PGRST205). Para salvar no Supabase permanentemente, copie o Script SQL abaixo e execute no SQL Editor do seu Supabase.`
        });
        setIsSyncing(false);
        return;
      }
      ...
      setSyncFeedback({
        ...
        message: isSupabaseConfigured 
          ? `🎉 Sucesso! ${validRows.length} revendedores parceiros foram sincronizados com o Supabase e habilitados no portal.`
          : `✓ ${validRows.length} revendedores foram adicionados à lista da plataforma com sucesso.`
      });
  ...
  const generatedSql = parsedRows.filter(r => r.status !== 'invalid_email').map(r => {
    ...
    const safeSpec = r.specialty ? r.specialty.replace(/'/g, "''") : 'Varejo Especializado';
    const safeCoupon = r.patientCoupon.replace(/'/g, "''");
    return `INSERT INTO profiles (name, trade_name, email, phone, city, state, specialty, role, wholesale_tier, store_type, patient_coupon, coupon_code, total_points, tier, updated_at)
VALUES ('${safeName}', '${safeName}', '${safeEmail}', '${safePhone}', '${safeCity}', '${safeState}', '${safeSpec}', 'REVENDEDOR', 'Ouro', 'Empório & Produtos Naturais', '${safeCoupon}', '${safeCoupon}', 0, 'Ouro', NOW())
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-3xl border border-[#2E4030]/15 shadow-2xl overflow-hidden my-8 max-h-[90vh] flex flex-col">
        ...
        <div className="bg-gradient-to-br from-[#1C261D] via-[#243325] to-[#121A13] p-6 sm:p-8 text-[#F7F3EC] flex items-start justify-between gap-4 shrink-0">
          <div className="space-y-1.5">
            ...
            <h2 className="text-xl sm:text-2xl font-serif font-bold text-white">
              Importar Lista de Lojistas & Revendedores (Excel / CSV)
            </h2>
            <p className="text-xs sm:text-sm text-[#C2C9C0] max-w-2xl">
              Faça o upload da sua planilha de parceiros comerciais com <strong>Loja, Responsável, Telefone, E-mail e Cupom de Bonificação</strong>. Os cadastros serão sincronizados diretamente no <strong>Supabase</strong>.
            </p>
          </div>
        ...
        <div className="p-6 sm:p-8 space-y-6 overflow-y-auto flex-1 text-primary-text">
          ...
          <div className={`p-4 rounded-2xl border flex items-center justify-between gap-3 text-xs ${
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span>
                {isSupabaseConfigured ? (
                  <><strong>Supabase Conectado:</strong> Os lojistas e revendedores serão gravados na tabela <code>profiles</code> (campos <code>patient_coupon</code> e <code>coupon_code</code>).</>
                ) : (
                  <><strong>Modo Pré-visualização:</strong> Conexão local ativa. Os lojistas e revendedores serão cadastrados na sessão atual.</>
                )}
              </span>
            </div>
        ...
        <div className="bg-[#FAF7F2] p-4 sm:p-6 border-t border-[#2E4030]/10 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0">
          <div className="text-xs text-[#526054]">
            {parsedRows.length > 0 ? (
              <span><strong>{validCount}</strong> revendedores prontos para serem sincronizados com o banco de dados.</span>
            ) : (
            ...
            >
              {isSyncing ? (
                ...
                <>
                  ...
                  <span>Cadastrar ({validCount}) Revendedores na Base</span>
                </>
              )}
            </button>
          </div>
